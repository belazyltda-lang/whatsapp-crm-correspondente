 ...
 * Versão Super Compacta (Menos de 180 linhas - Sem Risco de Truncamento)
 */
...
const sessions = {}, BANCOS_LISTA = ["Itaú", "Caixa Econômica", "Bradesco", "Santander", "Banco do Brasil"];
...
app.get('/', async (req, res) => {
  ...
  if (!currentQRCodeData) return res.send(`<!DOCTYPE html><html><head><title>Carregando...</title><meta http-equiv="refresh" content="3"></head><body style="font-family:Arial;text-align:center;padding:50px;"><h2>⏳ Gerando QR Code...</h2><p>Se demorar mais de 10s, <a href="/logout">clique aqui para resetar</a>.</p></body></html>`);
  try {
...
async function enviarDadoAtual(from, s) {
  switch (s.step) {
    case 'AGUARDANDO_NOME_PROP1': await sock.sendMessage(from, { text: `👤 Digite o *Nome Completo do Cliente (Proponente 1)*:` }).catch(()=>{}); break;
    case 'AGUARDANDO_CPF_PROP1': await sock.sendMessage(from, { text: `💳 Digite o *CPF do Proponente 1*:` }).catch(()=>{}); break;
    case 'AGUARDANDO_TEL_PROP1': await sock.sendMessage(from, { text: `📱 Digite o *Telefone do Proponente 1*:` }).catch(()=>{}); break;
    case 'AGUARDANDO_BANCO': await sock.sendMessage(from, { text: `🏦 Selecione a *Financeira / Banco desejado*:\n\n1️⃣ Itaú\n2️⃣ Caixa Econômica\n3️⃣ Bradesco\n4️⃣ Santander\n5️⃣ Banco do Brasil` }).catch(()=>{}); break;
    case 'AGUARDANDO_DOCS_PROP1': await sock.sendMessage(from, { text: `📷 Envie os *documentos* do Proponente 1 ou digite *PRONTO*:` }).catch(()=>{}); break;
    case 'AGUARDANDO_COMPRA_VENDA': await sock.sendMessage(from, { text: `📌 Digite o *Valor de Compra e Venda* (ex: 500.000):` }).catch(()=>{}); break;
    case 'AGUARDANDO_FINANCIAMENTO': await sock.sendMessage(from, { text: `Digite o *Valor do Financiamento* (ex: 400.000):` }).catch(()=>{}); break;
    case 'AGUARDANDO_ENTRADA': await sock.sendMessage(from, { text: `Digite o *Valor da Entrada* (ex: 100.000):` }).catch(()=>{}); break;
  }
}
    ...
    sock.ev.on('messages.upsert', async (m) => {
      try {
        ...
        if (['corrigir','editar','alterar'].includes(textLow) && s.step !== 'CONFIRMACAO_FINAL' && s.step !== 'EDITANDO_BLOCO_TEXTO') {
          await sock.sendMessage(from, { text: "👍 *Fique tranquilo! Você poderá conferir e corrigir todos os dados ao final do cadastro.*" }).catch(()=>{});
